...
benchmark_linear = 1; %1 if you want the size plot to use the linear (no non-linear regressor) as benchmark. Only applies if non_linearity = 2;
...
% if you choose 0, then pick in below which variables to use as controls

original_data = 1; %1 loads Kanzig's original data, 0 an updated data base. 

% For Kanzig's data, the variable order is:
...
trend = 0; %1: add constant and a linear trend, 0: add only a constant. 

do_weights = 1; % 1 if you want to compute and plot the weights

do_triple_test = 0; % 1 if you want to compute the triple test for symmetry of the shock.
...
har = 1; % 1 means Newey-West, 0 means just use heteroskedasticiy consistent as suggested by Plagborg-Moeller and Montiel Olea (ECTA, 2022) 
